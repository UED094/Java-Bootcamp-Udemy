...
public class Bank {
    ...
    private void withdrawTransaction(Transaction transaction) {
        if (this.getAccount(transaction.getId()).withdraw(transaction.getAmount())) {
    ...
    private void depositTransaction(Transaction transaction) {
        this.getAccount(transaction.getId()).deposit(transaction.getAmount());
        this.addTransaction(transaction);
    }
    ...
    // List list = this.transactions.stream()
    // .filter((transaction) -> transaction.getId().equals(account.getId()))
    // .findAny()
    // .map((transaction) -> {
    // if (transaction.getType() == Transaction.Type.WITHDRAW) {
    // return (double) (transaction.getAmount()) * (-1);
    // } else if (transaction.getType() == Transaction.Type.DEPOSIT) {
    // return (double) transaction.getAmount();
    // }
    // });
    // }
